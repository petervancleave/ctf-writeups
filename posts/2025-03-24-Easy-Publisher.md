...
# CTF Challenge Writeup: Publisher
...
Obviously, SQLi is not applicable to this challenge, but I thought of testing for itregardless just for fun and as an opportunity to try out SQLMap. SQLi occurs when user inputs in the form fields aren't properly sanitized, allowing attackers to manipulate SQL queries and bypass authentication.

I tried to use SQLi on the `/spip/ecrire` page because it had a login portal. 

I decided to use a high risk and level value just because this was my first time trying to utilize SQLi and wanted to see if I could get it to work for this. Also, I had to go do something so I just decided to let it run for a long time. Normally a level value of 1 and a risk value of 2 would suffice.

The level parameter controls the depth of the scan, determining which injection points are tested. the level values range 1-5.
1: Test GET and POST parameters
2: Tests HTTP cookies
3: Tests HTTP User-Agent and Referer headers
4: Tests HTTP Host headers
5: Tests HTTP headers and also attempts second order injections

Risk levels also go 1-5.
1: Default. Most tests are safe and generally non-intrusive
2: Includes some heavier tests that might generate warnings or errors
3: Includes more aggressive tests, which could cause application instability
4: Includes the most aggressive tests, which are very likely to disrupt the application
5: Includes the most aggressive tests and will cause the most disruption.


I tried some basic SQLi payloads:
...
These attempts didn't work, so I decided to automate the testing with SQLMap, a powerful tool for detecting and exploiting SQLi vulnerabilities.

**Using SQLMap:**  
I ran SQLMap to check for SQL injection vulnerabilities in the login page:

```bash
sqlmap -u "http://10.10.176.47/spip/spip.php?page=login&url=/spip/ecrire/&lang=en" --data="login*" --random-agent --batch --level=5 --risk=3 --dump
```

**Why I chose this approach:**  
SQLMap automates the detection of SQL injection flaws and can also attempt to extract data from the database if the vulnerability is found. I opted for a high `level=5` and `risk=3` setting to increase the chances of discovering a deeper vulnerability, even though these options might cause instability or errors in the application. This was a more aggressive approach because I had the time and curiosity to explore SQLi in depth (I had to go do something so I just decided to let it run because I wanted to see if I could get anything haha).

**No Results:**  
Unfortunately, after running SQLMap, no results were found. This didn't deter me but redirected my focus back to the initial goal: exploiting the SPIP RCE vulnerability.